...
function StatCard({ title, value }: { title: string; value: string }) {
  return (
    <motion.div variants={itemVariants} className="rounded-xl border border-(--accent)/25 bg-(--surface-muted) p-3">
...
export default function StatsSidebar() {
  ...
  return (
    <motion.aside 
      ...
      className="h-auto overflow-visible xl:h-fit xl:max-h-[calc(100svh-8.5rem)] xl:overflow-y-auto p-5 bg-(--surface-card) backdrop-blur-sm border-2 border-(--accent) rounded-2xl xl:sticky xl:top-0 xl:col-start-3 custom-scrollbar">
