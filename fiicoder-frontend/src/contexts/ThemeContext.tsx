...
// Themes whose base palette is light-toned (used to retarget low-contrast
// Tailwind palette utilities like text-amber-400, bg-red-500/15, etc.).
...
const MCDONALDS_FAVICON_SVG = `<svg viewBox="0 0 120 100" xmlns="http://www.w3.org/2000/svg"><path d="M 15 90 L 15 35 Q 15 12 37 12 Q 60 12 60 38 L 60 60 L 60 38 Q 60 12 83 12 Q 105 12 105 35 L 105 90 L 88 90 L 88 40 Q 88 26 76 26 Q 65 26 65 42 L 65 90 L 55 90 L 55 42 Q 55 26 44 26 Q 32 26 32 40 L 32 90 Z" fill="#FFC72C" stroke="#DA291C" stroke-width="2.5" stroke-linejoin="round"/></svg>`;

// Perceived brightness (ITU-R BT.601); > 128 ⇒ treat the background as light.
function isLightBackgroundHex(hex: string): boolean {
    ...
    return (redChannel * 299 + greenChannel * 587 + blueChannel * 114) / 1000 > 128;
}

/**
 * Applies the fully-custom theme (colors, text tone, cursors, radius, font,
 * border) to the document root. Returns whether the resolved tone is light.
 */
function applyCustomTheme(root: HTMLElement, customColors: CustomColors): boolean {
    root.style.setProperty('--bg-color', customColors.bg);
    root.style.setProperty('--accent', customColors.accent);

    const isLightBackground = isLightBackgroundHex(customColors.bg);
    const textHex = isLightBackground ? '#2d2530' : '#e5e9f0';
    const textHHex = isLightBackground ? '#120c14' : '#ffffff';
    root.style.setProperty('--text', textHex);
    root.style.setProperty('--text-h', textHHex);
    root.style.setProperty('--color-scheme', isLightBackground ? 'light' : 'dark');
    ...
    root.style.setProperty('--cursor-default', `url("data:image/svg+xml;charset=utf-8,${encodeURIComponent(cursorDefault)}")`);
    root.style.setProperty('--cursor-pointer', `url("data:image/svg+xml;charset=utf-8,${encodeURIComponent(cursorPointer)}")`);
    root.style.setProperty('--cursor-text', `url("data:image/svg+xml;charset=utf-8,${encodeURIComponent(cursorText)}")`);

    root.setAttribute('data-custom-radius', customColors.radius);

    const fontCfg = FONT_MAP[customColors.font];
    root.style.setProperty('--sans', fontCfg.sans);
    root.style.setProperty('--heading', fontCfg.heading);
    if (customColors.font === 'mono' || customColors.font === 'pixel') {
        root.style.setProperty('--mono', fontCfg.sans);
    } else {
        root.style.removeProperty('--mono');
    }

    root.setAttribute('data-custom-border', customColors.border);
    return isLightBackground;
}

/** Removes every custom-theme property/attribute, reverting to the theme's CSS. */
function clearCustomTheme(root: HTMLElement): void {
    ...
    root.removeAttribute('data-custom-radius');
    root.removeAttribute('data-custom-border');
    root.style.removeProperty('--sans');
    root.style.removeProperty('--heading');
    root.style.removeProperty('--mono');
}

function buildCustomFaviconSvg(accent: string): string {
    return `<svg viewBox="2.5229450154783466 65.87558809587485 252.37708872055313 121.06515820573098" xmlns="http://www.w3.org/2000/svg">
                <path style="fill: color-mix(in srgb, ${accent} 65%, white 35%)" fill-rule="evenodd" d="m17.3 109.25c-13.11 9.81-11.44 18.37-3.9 31.5 10.98 19.14 27.93 26.49 49.35 31.28 23.25 5.2 56.87-25.08 73.85-38.63 9.92-7.92 18.84-16.75 27.13-26.28 1.01-1.16 7-5.69 6.74-7.59-1.23-9-21.08-15.19-27.74-17.53-25.2-8.88-67.63-9.77-92.39 0.84-11.77 5.05-26.1 11.77-34.93 21.32-2.89 3.12-6.21 7.03-6.98 11.4-0.13 0.75 0.67 6.65-0.55 5.4"/>
...
function applyFavicon(theme: Theme, customColors: CustomColors): void {
    ...
    if (theme === 'mcdonalds') {
        favicon.href = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(MCDONALDS_FAVICON_SVG)}`;
    } else if (theme === 'custom') {
        favicon.href = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(buildCustomFaviconSvg(customColors.accent))}`;
    } else {
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
        ...
        // Resolve effective tone (light/dark). The custom theme derives it from
        // the chosen background; all other themes use the static classification.
        let isLightTone = STATIC_LIGHT_THEMES.includes(theme);
        if (theme === 'custom') {
            isLightTone = applyCustomTheme(root, customColors);
        } else {
            clearCustomTheme(root);
        }

        root.setAttribute('data-tone', isLightTone ? 'light' : 'dark');
        applyFavicon(theme, customColors);
    }, [theme, customColors]);
