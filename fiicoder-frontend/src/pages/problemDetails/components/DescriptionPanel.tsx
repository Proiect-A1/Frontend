...
// Plugin arrays are static — hoist them so their identity never changes across
// renders (react-markdown re-runs the pipeline when these change identity).
...
// The renderer map closes over `lang` only (via the CodeBlock `pre` override).
// Built once per language so the markdown + KaTeX pipeline isn't re-run when the
// component re-renders for unrelated reasons (e.g. a live evaluation update in a
// sibling panel). Kept identical to the previous inline map.
function makeMarkdownComponents(lang: string) {
    return {
        h1: ({ ...props }) => (
            <h1
...
function DescriptionPanel({ problem, processedDescription, lang }: Props) {
    const t = translations[lang as 'RO' | 'EN'] ?? translations.RO;
    // Normalize `** text **` → `**text**` so that backend-generated bold with
    // extra spaces (invalid CommonMark) still renders correctly.
    const normalized = useMemo(
        () => processedDescription.replace(/\*\* ?(.+?) ?\*\*/g, '**$1**'),
        [processedDescription],
    );

    const markdownComponents = useMemo(() => makeMarkdownComponents(lang), [lang]);
